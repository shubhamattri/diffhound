#!/bin/bash
# PR Review — Nova SDE-4 Full Policy + Multi-Agent Verification
# Usage: ./review-pr.sh <PR_NUMBER> [--auto-post] [--fast]
# Flow: Pass 1 Claude agentic (engineering findings) → Pass 2 Codex+Gemini parallel (verify, skipped in --fast)
#       → Pass 3 Haiku cached (merge + style rewrite in Shubham's voice) → Post
# Cost: ₹0 (Claude Pro CLI) + OpenAI/Gemini API costs for Pass 2
...
# ── Platform detection ───────────────────────────────────────
# macOS ships BSD awk (no 3-arg match) and uses $_TIMEOUT_CMD (GNU coreutils).
# Linux ships GNU awk and uses timeout natively.
if [ "$(uname -s)" = "Darwin" ]; then
  _TIMEOUT_CMD="$_TIMEOUT_CMD"
  _AWK_CMD="gawk"
...
# PARSE OUTPUT
# ============================================================
if grep -q "INLINE_COMMENTS_START" "$REVIEW_STRUCTURED"; then
  # Extract COMMENT:/REPLY: blocks, joining multi-line comment bodies onto one line.
  # LLM outputs multi-line comments — grep only captured the metadata line, losing the body.
  # Uses Unit Separator (\x1f) to join continuation lines; converted back to \n before posting.
  sed -n '/INLINE_COMMENTS_START/,/INLINE_COMMENTS_END/p' "$REVIEW_STRUCTURED" | \
    awk '
      /^COMMENT:|^REPLY:/ {
        if (buf != "") print buf
        buf = $0
    ...
    ' > "${REVIEW_STRUCTURED}.comments.raw" || true
  ...
  grep -viE '(trailing newline|missing newline|extra blank line|end of file|trailing whitespace|import order|file.ending|no newline at end)' \
    "${REVIEW_STRUCTURED}.comments.raw" > "${REVIEW_STRUCTURED}.comments" 2>/dev/null || \
    cp "${REVIEW_STRUCTURED}.comments.raw" "${REVIEW_STRUCTURED}.comments"
  rm -f "${REVIEW_STRUCTURED}.comments.raw"
else
  touch "${REVIEW_STRUCTURED}.comments"
fi

if grep -q "SUMMARY_START" "$REVIEW_STRUCTURED"; then
  sed -n '/SUMMARY_START/,/SUMMARY_END/p' "$REVIEW_STRUCTURED" | \
    grep -v "SUMMARY_START" | grep -v "SUMMARY_END" > "$REVIEW_SUMMARY"
else
  cat "$REVIEW_STRUCTURED" > "$REVIEW_SUMMARY"
fi
...
if [ "$POST_REVIEW" = true ]; then
  ...
  # Parse verdict — 3-method fallback for robust detection
  REVIEW_EVENT=""

  # Method 1: Extract verdict word from scorecard **Total** row
  _verdict_word=$(grep -i '\*\*Total\*\*' "$REVIEW_SUMMARY" | grep -oiE 'REQUEST_CHANGES|APPROVE|COMMENT' | head -1 || true)
  if [ -n "$_verdict_word" ]; then
    REVIEW_EVENT=$(echo "$_verdict_word" | tr '[:lower:]' '[:upper:]')
  fi

  # Method 2: Parse numeric score from **Total** row
  if [ -z "$REVIEW_EVENT" ]; then
    _total_score=$(grep -i '\*\*Total\*\*' "$REVIEW_SUMMARY" | grep -oE '[0-9]+/100' | grep -oE '^[0-9]+' || true)
    if [ -n "$_total_score" ]; then
      if [ "$_total_score" -ge 90 ]; then
        REVIEW_EVENT="APPROVE"
      elif [ "$_total_score" -lt 85 ]; then
  ...
  if [ -z "$REVIEW_EVENT" ]; then
    _has_blocking=$(grep -ci ':BLOCKING' "${REVIEW_STRUCTURED}.new_comments" 2>/dev/null || echo "0")
    _has_shouldfix=$(grep -ci ':SHOULD-FIX\|:SHOULD_FIX' "${REVIEW_STRUCTURED}.new_comments" 2>/dev/null || echo "0")
    if [ "$_has_blocking" -gt 0 ]; then
      REVIEW_EVENT="REQUEST_CHANGES"
    elif [ "$_has_shouldfix" -gt 0 ]; then
      REVIEW_EVENT="COMMENT"
    else
      REVIEW_EVENT="APPROVE"
    fi
  ...
  # GitHub rejects comments on lines not in the diff. This function
  # finds the nearest `+` line in the diff for a given file and line.
  _snap_to_diff_line() {
    local file="$1" target_line="$2"
    # Extract valid + line numbers for this file from the diff
    local valid_lines
    valid_lines=$(awk -v f="$file" '
      ...
      in_file && /^@@ / {
        # Parse @@ -old,count +new,count @@ (POSIX-compatible — no 3-arg match)
        s = $0
        sub(/.*\+/, "", s)
        sub(/,.*/, "", s)
        cur_line = int(s) - 1
...
JSONSTART
  ...
  if [ "$NEW_COMMENT_COUNT" -gt 0 ]; then
    ...
    while IFS=: read -r filepath line rest; do
      ...
      line="${line#\~}"  # strip leading ~ from approximate line numbers
      [[ ! "$line" =~ ^[0-9]+$ ]] && continue
      # Snap line to nearest valid diff line
      line=$(_snap_to_diff_line "$filepath" "$line")
      # Strip severity label prefix ("BLOCKING — ", "SHOULD-FIX — ", "NIT — ", etc.)
      # The COMMENT: format is "path:LINE:SEVERITY — body". We want only the body.
      # Handles em-dash (—), en-dash (–), or hyphen (-) as separator + underscore in SHOULD_FIX.
      # Also handles multi-line comments joined with \x1f by the awk extractor.
      comment=$(printf '%s' "${rest}" | sed -e 's/^[A-Z][A-Z_-]* [—–-] *//' -e "s/^[A-Z][A-Z_-]*$(printf '\x1f')//" | tr $'\x1f' '\n')
      # Skip if comment body is empty (only had severity label, no actual content)
...
JSONEND
  ...
  if ! gh api \
    ...
    if gh api \
      ...
      # Body posted, now post inline comments individually as review comments
      INLINE_POSTED=0
      if [ "$NEW_COMMENT_COUNT" -gt 0 ]; then
        while IFS=: read -r filepath line rest; do
          [[ ! "$filepath" =~ ^[a-zA-Z0-9/_.-]+$ ]] && continue
  ...
  if [ "$POSTED_OK" = true ]; then
    # Post REPLY comments to existing threads (separate API call per reply)
    if [ "$REPLY_COUNT" -gt 0 ]; then
      REPLY_POSTED=0
      while IFS=: read -r comment_id filepath line rest; do
        reply_body="${rest}"
        [[ ! "$comment_id" =~ ^[0-9]+$ ]] && continue
        # Post as a reply to the existing comment thread
        REPLY_BODY_JSON=$(echo "$reply_body" | jq -Rs .)
        if gh api \
          --method POST \
          -H "Accept: application/vnd.github+json" \
          -H "X-GitHub-Api-Version: 2022-11-28" \
      ...
      done < "${REVIEW_STRUCTURED}.replies"
  ...
  # ── VOICE INDEXER: Append posted comments to JSONL for continuous learning ──
  VOICE_JSONL_INDEX="$HOME/.ai_rules/review-voice-examples.jsonl"
  if [ -f "${REVIEW_STRUCTURED}.new_comments" ] && [ -f "$VOICE_JSONL_INDEX" ]; then
    INDEXED=0
    while IFS= read -r comment_line; do
      # Format: COMMENT: path:LINE:SEVERITY — text
      # Accept em-dash (—), en-dash (–), or hyphen (-) as separator (LLM output varies)
      [[ "$comment_line" =~ ^COMMENT:\ (.+):[~]?([0-9]+):(BLOCKING|SHOULD-FIX|NIT)\ [—–-]\ (.+)$ ]] || continue
      filepath="${BASH_REMATCH[1]}"
      severity="${BASH_REMATCH[3]}"
      comment_text="${BASH_REMATCH[4]}"

      # Skip very short comments (< 50 chars) — not worth indexing
      [ "${#comment_text}" -lt 50 ] && continue

      # Auto-detect category from comment content
      ...
      # Append as JSONL entry — use jq for safe JSON construction (handles
      # backslashes, quotes, control chars correctly; printf/sed cannot).
      jq -n \
        --arg cat "$cat" \
        --arg sub "$subcat" \
        --arg ext "$file_ext" \
