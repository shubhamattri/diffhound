...
15. **NO LAZY VERIFICATION — do the work yourself, never push it back to the author**:
    ...
    VULNERABILITY-TARGET-EXISTS RULE (anti-hallucination): Before asserting that a named code element — a resolver, endpoint, GraphQL query/mutation, route, handler, or function — is vulnerable, unscoped, exposed, missing an auth/access check, or otherwise insecure, you MUST first confirm that element actually EXISTS in the code under review (locate its definition or registration). NEVER infer a "companion"/"sibling" resolver or endpoint by analogy to another one (e.g. seeing a `users` resolver and assuming a `usersCount` resolver exists) and then claim the inferred element is vulnerable. If you cannot locate the element's definition, do NOT raise it as a BLOCKING or SHOULD-FIX security finding — at most raise an OPEN_QUESTION phrased as a question ("is there a `usersCount` resolver, and if so is it client-scoped?"). Asserting a security hole on an element you have not confirmed exists is a false positive. Real incident (monorepo #7268): claimed a `usersCount` resolver was unscoped when no such resolver exists anywhere in the repo.
...
- usage: {"type":"usage","subject":"<identifier>","scope":"<function/call it relates to>","expected":true|false}. Use when your finding asserts a VALUE/SYMBOL is or isn't passed to / used by / referenced within another function or call — e.g. "clientNames is never passed to getEffectiveClientIds", or "the flag check is bypassed because isOrgAdminPage returns first". expected:false = "subject is NOT used in/near scope". We verify by checking whether subject actually appears within scope's call/region. ⚠️ SAFETY: do NOT use `usage` to claim a SECURITY GUARD / authorization check is MISSING before some sink — a real missing-guard gap can have a drifted line cite and must NOT be auto-suppressed; raise those as ordinary findings. Reserve `usage` for plain data-flow "X flows into Y" assertions.
