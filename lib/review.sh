...
29. **Test-file anti-pattern sweep — when reviewing a *.spec.ts / *.test.ts file**:
    ...
    - Trivially-true ternary fallback assertions: `expect(actual).toBe(cond ? expected : actual)` reduces to `expect(actual).toBe(actual)` on the false branch, providing zero coverage. Same goes for `expect(value).toBe(value)`, `expect(arr.length).toBeGreaterThanOrEqual(0)` (always true for arrays), `expect(value).toBeDefined()` after a non-null assertion (`!`), and `expect(typeof x).toBe(typeof y)` where x and y are literally the same expression. Flag with the exact line and the simpler direct assertion.
    - Test description vs. behaviour mismatch: when a test's `it("...")` description names a specific code branch (e.g. "below threshold", "with cache hit", "when X is null") but the test setup does NOT exercise that branch, flag. Read the test's assertions and mock setup; the description must match the path actually traversed. Wrong descriptions mislead future readers about which branch is under test and erode coverage signal.
